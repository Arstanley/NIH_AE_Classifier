...
class MultipleClassAUROC(Callback):
    ...
    """
    ### NOTE: modified class to initialize model_epoch variable ### 
    def __init__(self, sequence, class_names, weights_path, stats=None, workers=1, model_epoch=0):
        ...
        self.best_weights_path = os.path.join(
            ...
            f"best_epoch{self.epoch}_{os.path.split(weights_path)[1]}",
        )
        self.best_auroc_log_path = os.path.join(
            os.path.split(weights_path)[0],
            f"best_auroc_for_epoch{self.epoch}.log",
        )
        self.stats_output_path = os.path.join(
            os.path.split(weights_path)[0],
            "{self.epoch}.training_stats.json"
        )
